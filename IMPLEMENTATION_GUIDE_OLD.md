...
#### **EKS Pod Identity Setup for Custom Applications**

If you need to set up EKS Pod Identity for your own applications, here's how:

```bash
# Example: Setting up EKS Pod Identity for a custom fraud detection service

# 1. Create IAM role for your application
aws iam create-role \
    --role-name FraudDetectionServiceRole \
    --assume-role-policy-document '{
        "Version": "2012-10-17",
        "Statement": [
            {
...
# 2. Create and attach custom IAM policy
cat > fraud-detection-policy.json << EOF
{
    "Version": "2012-10-17",
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:s3:::your-fraud-detection-bucket",
                "arn:aws:s3:::your-fraud-detection-bucket/*"
            ]
        },
        {
            ...
            "Resource": "arn:aws:secretsmanager:us-west-2:*:secret:fraud-detection/*"
        }
    ]
}
EOF
...
aws iam create-policy \
    --policy-name FraudDetectionServicePolicy \
    --policy-document file://fraud-detection-policy.json

aws iam attach-role-policy \
    --role-name FraudDetectionServiceRole \
    --policy-arn arn:aws:iam::$(aws sts get-caller-identity --query Account --output text):policy/FraudDetectionServicePolicy

# 3. Create EKS Pod Identity association
aws eks create-pod-identity-association \
    --cluster-name $CLUSTER_NAME \
    --namespace ml-team-a \
    --service-account fraud-detection-sa \
    --role-arn arn:aws:iam::$(aws sts get-caller-identity --query Account --output text):role/FraudDetectionServiceRole

# 4. Create Kubernetes service account (no annotations needed!)
kubectl create serviceaccount fraud-detection-sa -n ml-team-a

# 5. Deploy your application with the service account
cat > fraud-detection-deployment.yaml << EOF
apiVersion: apps/v1
kind: Deployment
metadata:
  ...
  template:
    metadata:
      labels:
        app: fraud-detection-service
    spec:
      serviceAccountName: fraud-detection-sa  # This is all you need!
      containers:
      - name: fraud-detection
        image: your-fraud-detection:latest
        env:
        - name: AWS_REGION
          value: us-west-2
        # No need for AWS_ROLE_ARN or other IRSA-specific env vars
EOF
...
if ! kubectl get deployment aws-load-balancer-controller -n kube-system &> /dev/null; then
    ...
    aws eks create-pod-identity-association \
        ...
        --namespace kube-system \
        --service-account aws-load-balancer-controller \
        --role-arn arn:aws:iam::$AWS_ACCOUNT_ID:role/AmazonEKSLoadBalancerControllerRole 2>/dev/null || echo "Association already exists"
...
### Modern AWS IAM Integration
This implementation uses **EKS Pod Identity** instead of the legacy IRSA (IAM Roles for Service Accounts) approach:

- **Simplified Configuration**: No need for OIDC provider setup or complex trust policies
- **Better Security**: Direct pod-to-role association without JWT tokens
- **Easier Management**: Centralized identity management through EKS
- **Improved Performance**: Faster credential retrieval and refresh

### Key Components Using EKS Pod Identity
- **AWS Load Balancer Controller**: Manages Application Load Balancers and Network Load Balancers
- **EBS CSI Driver**: Manages EBS volume lifecycle
- **EFS CSI Driver**: Manages EFS filesystem access
- **JupyterHub**: Secure access to S3 and EMR resources
- **External Secrets Operator**: Secure access to AWS Secrets Manager

### EKS Pod Identity vs IRSA Comparison
| Feature | IRSA (Legacy) | EKS Pod Identity (Modern) |
|---------|---------------|---------------------------|
| **Setup Complexity** | Complex OIDC setup | Simple role association |
| **Token Management** | JWT tokens with expiration | Automatic credential management |
| **Security** | Trust policy with conditions | Direct pod identity |
| **Performance** | Token refresh overhead | Optimized credential retrieval |
| **Debugging** | Complex token troubleshooting | Simplified identity validation |
...
4. **AWS Load Balancer Controller Missing**
   ...
   # Create IAM role and EKS Pod Identity association
   AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
